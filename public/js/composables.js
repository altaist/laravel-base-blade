...
const useAuth = () => {
    ...
    const checkAutoAuth = async () => {
        ...
        try {
            ...
            if (result.success) {
                ...
                // Токен недействителен - НЕ удаляем, оставляем как флаг
                return {
                    hasToken: true,
                    isValid: false,
                    token: token
    ...
    const initAutoAuth = async () => {
        ...
        // Сначала проверяем флаг автогенерации
        if (!wasAutoGenerated()) {
        ...
        // Проверяем токен автологина только если пользователь был автогенерирован
        ...
        if (authResult.hasToken && authResult.isValid) {
            ...
            console.log('Токен автологина недействителен или отсутствует');
            // Токен невалиден или отсутствует - ничего не делаем
        }
    };
