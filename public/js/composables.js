...
const useAuth = () => {
    ...
    const AUTO_GENERATED_FLAG = 'was_auto_generated';
    ...
    const api = {
        ...
        async createUser() {
            return await makeApiRequest('/api/auto-auth/create-user');
        }
    };
    ...
    const rejectAutoAuth = async (token) => {
        try {
            // Отправляем запрос на сервер для удаления из базы данных
            await api.rejectAuth(token);
            
            console.log('Автологин отклонен, токен удален с сервера');
            return { success: true };
        } catch (error) {
    ...
    /**
     * Проверить, был ли пользователь автогенерирован
     */
    const wasAutoGenerated = () => {
        try {
            return localStorage.getItem(AUTO_GENERATED_FLAG) === '1';
        } catch (error) {
            console.warn('Ошибка чтения флага автогенерации:', error);
            return false;
        }
    };

    /**
     * Установить флаг автогенерации
     */
    const setAutoGeneratedFlag = () => {
        try {
            localStorage.setItem(AUTO_GENERATED_FLAG, '1');
            console.log('Флаг автогенерации установлен');
        } catch (error) {
            console.error('Ошибка установки флага автогенерации:', error);
        }
    };
    ...
    const createNewUser = async () => {
        try {
            ...
            if (result.success) {
                console.log('Пользователь создан, сохраняем токен...');
                // Сохраняем токен в localStorage
                const saved = storageUtils.set(STORAGE_NAME, result.token, STORAGE_EXPIRES);
                if (saved) {
                    console.log('Токен автологина сохранен, устанавливаем флаг автогенерации...');
                    // Устанавливаем флаг автогенерации
                    setAutoGeneratedFlag();
                    console.log('Авторизуем пользователя...');
                    // Сразу авторизуем пользователя БЕЗ popup
                    await confirmAutoAuth(result.token);
                } else {
                    console.error('Не удалось сохранить токен автологина');
                }
            } else {
    ...
    const initAutoAuth = async () => {
        ...
        } else if (!authResult.hasToken) {
            console.log('Токен автологина не найден, проверяем флаг автогенерации...');
            // Нет токена - проверяем флаг для регистрации
            if (!wasAutoGenerated()) {
                console.log('Пользователь не был автогенерирован, создаем нового пользователя...');
                await createNewUser();
            } else {
                console.log('Пользователь был автогенерирован, ничего не делаем');
            }
        } else {
            console.log('Токен автологина недействителен, ничего не делаем');
