...
class TelegramAuthController extends Controller
{
    ...
    public function callback(TelegramAuthRequest $request)
    {
        ...
        $user = User::whereHas('telegramBots', function($query) use ($request) {
            $query->where('telegram_id', $request->validated('id'))
                  ->where('bot_name', 'main');
        })->first();
        ...
        if (!$user) {
            ...
            // Привязываем к боту
            $user->telegramBots()->create($botData);
            ...
            $user->telegramBots()
                ...
                ->update([
                    'telegram_id' => $request->validated('id'),
                    'bot_data' => ['username' => $request->validated('username')],
                ]);
        }
