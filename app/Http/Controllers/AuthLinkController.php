...
class AuthLinkController extends Controller
{
    ...
    private function getOrCreateUser(AuthLink $authLink): ?User
    {
        ...
        Log::info('Пользователь не найден в ссылке: ', ['authLink' => $authLink]);
    ...
    {
        Log::info('Начинаю создавать пользователя из ссылки: ', ['authLink' => $authLink]);
        ...
        try {
            ...
            // Обновляем user_id в ссылке, чтобы при повторном нажатии пользователь был найден
            if ($user && $authLink->isForRegistration()) {
                $authLink->update(['user_id' => $user->id]);
                Log::info('Обновлен user_id в ссылке регистрации', [
                    'auth_link_id' => $authLink->id,
                    'user_id' => $user->id
                ]);
            }
            ...
            Log::error('Критическая ошибка при создании пользователя из ссылки: ' . $e->getMessage());
            return null;
        }
    }
